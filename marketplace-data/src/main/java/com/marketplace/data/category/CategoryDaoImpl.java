...
public class CategoryDaoImpl implements CategoryDao {
    ...
    public Category save(CategoryInput values) {
        ...
        if (values.getNames() != null) {
        	var names = values.getNames().stream().map(n -> {
        		var en = new CategoryNameEntity();
        		en.getId().setLang(n.getLang());
        		en.setCategory(result);
        		en.setName(n.getName());
        		return en;
        	}).toList();
        	nameRepo.saveAll(names);
        }
    ...
    public List<Category> findRootCategories() {
        return repo.findByCategoryNull().stream()
                .map(CategoryMapper::toDomainCompat)
    ...
    public List<Category> findAll(boolean withNames) {
        return repo.findAll().stream()
                .map(e -> CategoryMapper.toDomain(e, withNames))
                .collect(Collectors.toList());
    }
